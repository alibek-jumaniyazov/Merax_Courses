    ...
    <div className="CourseRequired w-full flex flex-col justify-center items-center gap-12 mt-24 mainFont bg-color">
      ...
      <div className="w-full flex flex-wrap justify-center items-center gap-8">
        {required.map((item, index) => (
          <div
            className="w-[40%] flex justify-start items-center gap-5 m-1 border-[2px] border-mainColor  hover:bg-white hover:border-transparent hover:shadow-xl p-9 rounded-3xl transition-all duration-200 cursor-pointer"
            key={index}
          >
            <Image src={item.image} alt="info Img" />
            <div className="flex flex-col justify-center items-start gap-2">
              <p className="font-semibold text-[22px]">{item.title}</p>
              <span className="font-normal text-mainColor">{item.info}</span>
            </div>
          </div>
        ))}
      </div>
